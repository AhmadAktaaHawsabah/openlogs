...
function header(text) {
  ...
  console.log(`${colors.bright}${colors.cyan}║ ${text.padEnd(37)} ║${colors.reset}`);
...
async function runExamples() {
  header("OpenLogs SDK v2.0 Test Suite");
  ...
  const signed1 = await signV2Record(record1, {
    ...keys,
  ...
  const signed2 = await signV2Record(record2, {
    ...keys,
    kid: "boy:audit-key-2026",
  });
  success("Record 2 signed");

  const signed3 = await signV2Record(record3, {
    ...keys,
    kid: "boy:audit-key-2026",
  });
  ...
  const verifyResult = await verifyV2Chain([signed1, signed2, signed3]);
  if (verifyResult.ok) {
    ...
    error(`Verification failed: ${verifyResult.error} at index ${verifyResult.index}`);
  }
  divider();
  ...
  const geoRecord = createV2Record(
    {
  ...
  const tpsString = "tps://L:40.7128,-74.0060@T:greg.m3.c1.y26.m3.d3.h14.m30.s0.m0";
  const tpsuid = generateTpsUid(tpsString);
  ...
  // ─────────────────────────────────────────────────────────────────────────
  header("Test Summary");
  console.log(`${colors.bright}${colors.green}✅ All 8 examples executed successfully${colors.reset}`);
  console.log(
    `
